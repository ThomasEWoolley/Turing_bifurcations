...
lx=1; nx=100;% Space setup and number of elements to be used, space is [-lx,lx]X[-ly,ly].
...
p.plot.bpcmp=2;
p.nc.dsmax=0.01; % Arclength max steplength
p.nc.lammax=0.4; % Max parameter value
p=cont(p,100);
...
%%
close
plotbra('hom1D','bplab',[1:5],'lp',400); 
plotbra('1D1','bplab',[1:5],'lp',400); 
plotbra('1D2','bplab',[1:5],'lp',400); 
plotbra('bp1_from1D1','bplab',[1:5],'lp',400); 
% %%
% 
% 
% Checked=[];
...
r=[1e-3*K/L^2 N; N 1e-1*K/L^2]*[u1;u2]-p.mat.M*f+1e2*[Qu*u1-bcu;Qv*u2-bcv]; % putting the equation together
end
