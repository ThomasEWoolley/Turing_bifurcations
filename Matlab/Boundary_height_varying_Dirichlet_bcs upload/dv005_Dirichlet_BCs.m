...
p=oosetfemops(p);
p.nc.nsteps=50;
p=setfn(p,'hom1D005',0.01);
p=cont(p,100);
%%
 huclean(p); % Set up figures in nice way
p=swibra('hom1D005','bpt1','1D1005',-0.001);
p.plot.bpcmp=4;
...
%%
huclean(p); % Set up figures in nice way
p=swibra('hom1D005','bpt1','1D2005',0.001);
p.plot.pcmp=1; % Component for plotting
p.plot.bpcmp=4;
p.nc.dsmax=0.1; % Arclength max steplength
p.nc.lammax=2; % Max parameter value
...
r=[1e-3*K/L^2 N; N 0.5e-1*K/L^2]*[u1;u2]-p.mat.M*f+2e4*[Qu*u1-bcu;Qv*u2-bcv]; % putting the equation together
end
