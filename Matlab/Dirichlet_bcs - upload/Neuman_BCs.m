...
par=           [ 0.05]; % Parameters used in functions and Jacobian
...
lx=1; nx=20;% Space setup and number of elements to be used, space is [-lx,lx]X[-ly,ly].
pde=stanpdeo1D(0.5,0.01); % PDE setup
...
p.nc.dlammax=0.2; % Maximum step in the parameter
...
p=oosetfemops(p);
p.nc.nsteps=50;
p=setfn(p,'Neumann_hom1D',0.01);
p=cont(p,50);
%%

p=swibra('Neumann_hom1D','bpt1','Neumann_1D1',0.001);
